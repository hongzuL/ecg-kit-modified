                                ...
                                try
                                    ...
                                    if( any(strcmpi( {'sqrs' 'wqrs' 'epltdqrs1' 'epltdqrs2'} , this_detector ) ) )
                                        ...
                                        if( status ~= 0 ); 
                                            ...
                                            % record the current ECG file
                                            disp('need rerun')
                                            namesplit=strsplit(ECG_header_WFDB.recname,'_');                  
                                            fileID = fopen('C:\Users\Hongzu\Documents\ECG_data\rerun.txt','a');
                                            fprintf(fileID,'%s\n',namesplit{1});
                                            fclose(fileID);
                                            disp_string_framed(2, sprintf('%s failed in recording %s lead %s', this_detector, ECG_header_WFDB.recname, ECG_header_WFDB.desc(jj,:) ) ); 
                                        end
